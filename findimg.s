...
findimg:
        ...
        sub	esp, 36         ; [ebp-4] width_diff
                                ...
                                ; [ebp-36] current_mask_for_full_check
...
full_check_pixel:
        ...
        mov     edx, esi           ; copy to_find image data to edx
        and     edx, [ebp-36]      ; apply mask to sub image data
        cmp     edi, edx           ; compare the two pixels
        jnz     reset_parameters    ; if not equal, end checking
        inc     dword[ebp-20]      ; current_x_offset++
        shr     dword[ebp-36], 1   ; shift the mask to the right
        jnz     full_check_pixel
        mov     dword[ebp-36], 0x80000000
        jmp     full_check_load  ; if mask is 0, reset it and load new data
